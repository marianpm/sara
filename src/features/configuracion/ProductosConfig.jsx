...
import ListaPreciosImagen from "./components/ListaPreciosImagen";
import { copiarNodoComoImagen } from "./utils/listaPreciosUtils";
...
export default function ProductosConfig({
  ...
  const listaMayoristaRef = useRef(null);
  const listaMinoristaRef = useRef(null);

  const [copiandoListaMayorista, setCopiandoListaMayorista] = useState(false);
  const [copiandoListaMinorista, setCopiandoListaMinorista] = useState(false);
  ...
  const exportarGoogleSheet = async () => {
    ...
    const copiarListaMayorista = async () => {
    if (copiandoListaMayorista) return;

    try {
      setCopiandoListaMayorista(true);
      await copiarNodoComoImagen(listaMayoristaRef.current);
      mostrarMensaje(
        "success",
        "La lista de precios mayorista se copió como imagen."
      );
    } catch (e) {
      mostrarMensaje("error", e.message || "No se pudo copiar la imagen.");
    } finally {
      setCopiandoListaMayorista(false);
    }
  };

  const copiarListaMinorista = async () => {
    if (copiandoListaMinorista) return;

    try {
      setCopiandoListaMinorista(true);
      await copiarNodoComoImagen(listaMinoristaRef.current);
      mostrarMensaje(
        "success",
        "La lista de precios minorista se copió como imagen."
      );
    } catch (e) {
      mostrarMensaje("error", e.message || "No se pudo copiar la imagen.");
    } finally {
      setCopiandoListaMinorista(false);
    }
  };
  ...
  const guardarProducto = async (productoId) => {
    ...
    const productoCambio =
      normalizarTexto(productoOriginal.nombre) !== nombre ||
      (productoOriginal.activo !== false) !== (producto.activo !== false);
    ...
    for (const variante of producto.producto_variantes || []) {
      ...
      if (!presentacion) {
        mostrarMensaje(
          "error",
          `La presentación del producto "${nombre}" no puede quedar vacía.`
        );
        return;
      }
      ...
      if (Number.isNaN(precioMayorista) || precioMayorista < 0) {
        mostrarMensaje(
          "error",
          `El precio mayorista de "${presentacion}" no es válido.`
        );
        return;
      }
      ...
      if (Number.isNaN(precioMinorista) || precioMinorista < 0) {
        mostrarMensaje(
          "error",
          `El precio minorista de "${presentacion}" no es válido.`
        );
        return;
      }

      const varianteCambio =
        normalizarTexto(varianteOriginal.presentacion) !== presentacion ||
        aNumero(varianteOriginal.precio_mayorista) !== precioMayorista ||
        aNumero(varianteOriginal.precio_minorista) !== precioMinorista ||
        (varianteOriginal.activo !== false) !== (variante.activo !== false);

      if (varianteCambio) {
        variantesAActualizar.push({
          varianteOriginal,
          varianteEditada: {
            ...variante,
            presentacion,
    ...
    if (!productoCambio && variantesAActualizar.length === 0) {
      mostrarMensaje("success", "No hay cambios para guardar.");
      return;
    }
    ...
    try {
      if (productoCambio) {
        ...
        if (updateProductoError) {
          mostrarMensaje("error", textoErrorSupabase(updateProductoError));
          return;
        }
      ...
      for (const item of variantesAActualizar) {
        const { id, ...payload } = item.updatePayload;
        ...
        if (updateVarianteError) {
          mostrarMensaje("error", textoErrorSupabase(updateVarianteError));
          return;
        }
        ...
        registrarLogsCambiosVariante({
          ...
          varianteOriginal: item.varianteOriginal,
          varianteEditada: item.varianteEditada,
        });
      }
      ...
      if (productoCambio) {
        partes.push("producto actualizado");
      }

      if (variantesAActualizar.length > 0) {
        partes.push(
          `${variantesAActualizar.length} ${
            variantesAActualizar.length === 1
      ...
      mostrarMensaje("success", `Cambios guardados correctamente: ${partes.join(", ")}.`);
    ...
    <div className="space-y-4">
          ...
          <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 space-y-3">
            <div className="flex flex-col gap-3">
              <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                <div>
                  <h3 className="font-medium">Exportar precios</h3>
                  <p className="text-sm text-slate-500">
                    Copiá el link, exportá a Google Sheet o copiá las listas como imagen.
                  </p>
                </div>
              ...
              <div className="flex flex-wrap items-center gap-2">
                <Button
                  type="button"
                  onClick={copiarListaMayorista}
                  disabled={copiandoListaMayorista || cargando || !!error}
                >
                  {copiandoListaMayorista
                    ? "Copiando..."
                    : "Copiar lista de precios Sarria (Mayorista)"}
                </Button>

                <Button
                  type="button"
                  variant="outline"
                  onClick={copiarListaMinorista}
                  disabled={copiandoListaMinorista || cargando || !!error}
                >
                  {copiandoListaMinorista
                    ? "Copiando..."
                    : "Copiar lista de precios 1319 (Minorista)"}
                </Button>
              </div>
            </div>
          </div>
      ...
      {!cargando &&
        ...
        <div className="fixed -left-[99999px] top-0 pointer-events-none opacity-0">
          <ListaPreciosImagen
            ref={listaMayoristaRef}
            productos={productosLocales}
            tipo="mayorista"
          />

          <ListaPreciosImagen
            ref={listaMinoristaRef}
            productos={productosLocales}
            tipo="minorista"
          />
        </div>
