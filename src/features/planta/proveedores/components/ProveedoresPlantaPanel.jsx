...
function formatKgConSigno(valor) {
  if (valor == null || Number.isNaN(Number(valor))) return "-";

  const numero = Number(valor);
  const signo = numero > 0 ? "+" : "";

  return `${signo}${numero.toLocaleString("es-AR", {
    minimumFractionDigits: 2,
    maximumFractionDigits: 2,
  })} kg`;
}
...
export default function ProveedoresPlantaPanel({ usuarioActual }) {
  ...
  const patasPesoInformadoProveedor = normalizarNumero(
    form.patasPesoInformadoProveedor
  );

  const patasPesoPromedioInformadoProveedor = calcularPesoPromedio(
    patasPesoInformadoProveedor,
    patasCantidadNumero
  );

  const diferenciaPesoInformadoProveedor =
    patasPesoInformadoProveedor != null &&
    patasPesoInformadoProveedor > 0 &&
    patasPesoTotalKg > 0
      ? patasPesoTotalKg - patasPesoInformadoProveedor
      : null;

  const diferenciaPesoInformadoClassName =
    diferenciaPesoInformadoProveedor == null ||
    Math.abs(diferenciaPesoInformadoProveedor) < 0.005
      ? "text-slate-900"
      : diferenciaPesoInformadoProveedor > 0
      ? "text-emerald-700"
      : "text-red-700";
  ...
  const limpiarFormulario = () => {
    ...
    setMostrarObservacionesIngreso(false);
    setMensajeOk(null);
    setErrorForm(null);
  };
  ...
  const guardarIngreso = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...
        observaciones: mostrarObservacionesIngreso
              ...
              <div className="grid gap-4 xl:grid-cols-[minmax(0,1.45fr)_minmax(260px,0.55fr)]">
                <div className="rounded-2xl border bg-white p-4 space-y-3">
                  ...
                  <div className="grid gap-3 md:grid-cols-3 md:items-end">
                    ...
                    <div className="space-y-1">
                      <label className="block text-sm font-medium leading-5">
                        Peso informado proveedor
                      </label>
                      <Input
                        type="number"
                        min="0"
                        step="0.01"
                        value={form.patasPesoInformadoProveedor}
                        onChange={(e) =>
                          setCampo("patasPesoInformadoProveedor", e.target.value)
                        }
                        placeholder="kg"
                      />
                    </div>
                    ...
                    <div className="space-y-1">
                      ...
                      <div className="flex h-10 w-full items-center rounded-md border border-slate-300 bg-slate-50 px-3 text-sm text-slate-900">
                        {formatKgPorPata(patasPesoPromedioInformadoProveedor)}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm">
                      ...
                      <div className="mt-1 flex justify-between gap-3">
                        <span className="text-slate-500">Diferencia vs informado</span>
                        <span className={`font-semibold ${diferenciaPesoInformadoClassName}`}>
                          {formatKgConSigno(diferenciaPesoInformadoProveedor)}
                        </span>
                      </div>

                      <div className="mt-1 flex justify-between gap-3">
                        <span className="text-slate-500">Promedio total pesado</span>
